...
<template>
  <div class="container">
    <div class="d-flex flex-column justify-content-center">
      <div
        class="cocktailCards"
        v-for="(cocktails, category, index) in cocktailsByCategory"
        :key="category"
      >
        <h2 class="mb-2">{{ category }}</h2>
        <div
          class="cocktailCard p-2 m-2"
          v-for="cocktail in cocktails"
          :key="cocktail.id"
          :style="{ backgroundColor: !cocktails.length ? 'white' : colors[index % colors.length] }"
        >
          <div class="d-flex align-items-center">
            ...
            <div class="d-flex justify-content-between w-100">
              ...
              <div class="d-flex flex-column">
                <button @click="addToCart(cocktail, 'L')" type="button" class="btn btn-info m-1">
                  Large: {{ cocktail.price_L }} €
                </button>
                <button @click="addToCart(cocktail, 'M')" type="button" class="btn btn-warning m-1">
                  Medium: {{ cocktail.price_M }} €
                </button>
                <button @click="addToCart(cocktail, 'S')" type="button" class="btn btn-success m-1">
                  Small: {{ cocktail.price_S }} €
                </button>
              </div>
            </div>
          </div>
